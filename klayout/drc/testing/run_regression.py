...
def convert_results_db_to_gds(results_database: str, rules_tested: list):
    ...
    for r in rule_data_type_map:
        ...
        fail_marker2.interacting(fail_marker.interacting(text_marker.texts("{r}")) ).join( fail_marker.interacting(text_marker.texts("{r}")).not_interacting(fail_marker2) ).output("{r}{RULE_STR_SEP}fail_patterns", "{r}{RULE_STR_SEP}fail_patterns polygons")
        """
        false_pos_rule = f"""
        ...
        """
        false_neg_rule = f"""
        ((fail_marker2.interacting(fail_marker.interacting(text_marker.texts("{r}")))).join((fail_marker.interacting(input(11, 222).texts("{r}")).not_interacting(fail_marker2)))).not_interacting({rule_layer_name}).output("{r}{RULE_STR_SEP}false_negative", "{r}{RULE_STR_SEP}false_negative occurred")
        """
        rule_not_tested = f"""
